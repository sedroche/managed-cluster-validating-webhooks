...
var (
	timeout        int32 = 2
	allowedUsers         = []string{"kube:admin", "system:admin", "backplane-cluster-admin", "system:serviceaccount:openshift-monitoring:prometheus-operator"}
	sreAdminGroups       = []string{"system:serviceaccounts:openshift-backplane-srep", "cluster-admins"}
...
func (s *prometheusruleWebhook) renderPrometheusRule(req admissionctl.Request) (*promRule, error) {
